...
```
 - При вводе из файла в первой строке должна быть казана длина строки, а во второй - подстроки. Сама строка и подстрока располагаются на новых строках.
...
### Тестирование ASM
 - Тест где будет найдена подстрока `./main ../tests/test1.txt out.txt`
 ```
 5
 2
 abcdef
...
```
 - Тест где не будет найдена подстрока `./main ../tests/test2.txt out.txt`
 ```
5
3
aaaaa
...
```
 - Тест где будет показано что выводится первое вхождение `./main ../tests/test3.txt out.txt`
 ```
